...
/* Protects fg[], bg[], and the dirty rect (x1/y1/x2/y2).
   Draw operations acquire a read lock so workers can draw concurrently,
   while redraw/palette/resize acquire a write lock for exclusive access. */
static pthread_rwlock_t screen_rwlock = PTHREAD_RWLOCK_INITIALIZER;

static void
atomic_min_int(int *ptr, int value)
{
	int current = __atomic_load_n(ptr, __ATOMIC_RELAXED);
	while(value < current &&
		!__atomic_compare_exchange_n(ptr, &current, value, 1, __ATOMIC_RELAXED, __ATOMIC_RELAXED))
		;
}

static void
atomic_max_int(int *ptr, int value)
{
	int current = __atomic_load_n(ptr, __ATOMIC_RELAXED);
	while(value > current &&
		!__atomic_compare_exchange_n(ptr, &current, value, 1, __ATOMIC_RELAXED, __ATOMIC_RELAXED))
...
screen_changed(void)
{
	int x1 = __atomic_load_n(&uxn_screen.x1, __ATOMIC_RELAXED);
	int y1 = __atomic_load_n(&uxn_screen.y1, __ATOMIC_RELAXED);
	int x2 = __atomic_load_n(&uxn_screen.x2, __ATOMIC_RELAXED);
	int y2 = __atomic_load_n(&uxn_screen.y2, __ATOMIC_RELAXED);
	return x2 > x1 && y2 > y1;
}
...
{
	atomic_min_int(&uxn_screen.x1, x1);
	atomic_min_int(&uxn_screen.y1, y1);
	atomic_max_int(&uxn_screen.x2, x2);
	atomic_max_int(&uxn_screen.y2, y2);
}
...
screen_palette(void)
{
	...
	pthread_rwlock_wrlock(&screen_rwlock);
	...
	pthread_rwlock_unlock(&screen_rwlock);
}
...
screen_deo(Uint8 addr)
{
	...
	case 0x2e: {
		...
		if(ctrl & 0x80) {
			...
			pthread_rwlock_rdlock(&screen_rwlock);

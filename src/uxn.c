...
uxn_eval(Uint16 pc)
{
	...
	while (1) {
		...
		/* LDZ */ OPC(0x10,PO1(a),PEK(a + uxn.dev[0xdA] * PAGE_SIZE, x, (PAGE_SIZE * BANKS - 1)))
		/* STZ */ OPC(0x11,PO1(a) GET(y),POK(a + uxn.dev[0xdA] * PAGE_SIZE, y, (PAGE_SIZE * BANKS - 1)))
		/* LDR */ OPC(0x12,PO1(a),PEK(pc + (Sint8)a, x, 0xffff))
		/* STR */ OPC(0x13,PO1(a) GET(y),POK(pc + (Sint8)a, y, 0xffff))
        /* LDA */ OPC(0x14,PO2(a),PEK(a + uxn.dev[0xdA] * PAGE_SIZE, x, (PAGE_SIZE * BANKS - 1)))
        /* STA */ OPC(0x15,PO2(a) GET(y),POK(a + uxn.dev[0xdA] * PAGE_SIZE, y, (PAGE_SIZE * BANKS - 1)))
